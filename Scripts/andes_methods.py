...
def limiter_to_indicator(mdl, device):
    ...
    for i in mdl.discrete:
        ...
        base_var = sp.Symbol(var_name + "_" + mdl_name + str(device))
...
def sys_to_eq(sys, mode = "dynamic", bool_print = True):
    ...
    for mdl in sys.models.values():
        ...
        for device in range(num_devices):
            ...
            algeb_vars_device = [var + "_" + mdl_name + "_" + str(device) for var in algeb_vars]
            state_vars_device = [var + "_" + mdl_name + "_" + str(device) for var in state_vars]
            ...
            if mdl.group in ["TurbineGov", "Exciter"]:
                ...
                for var in ext_vars:
                    ...
                    var_original = sp.Symbol(var + "_" + mdl_name + "_" + str(device))     
            ...
            for i, bus_id in enumerate(bus_ids):
                ...
                if mdl_name == "Bus" or not connected_to_bus:
                    break
                a_bus = sp.Symbol("a_Bus" + "_" +str(i))
                a_model = sp.Symbol("a_" + mdl_name + "_" + str(device))
                v_bus = sp.Symbol("v_Bus" + "_" +str(i)) 
                v_model = sp.Symbol("v_"+ mdl_name + "_" + str(device)) 
            ...
            x_dot = [str(s + "_" + mdl_name + "_" + str(device)) for s in x_dot]    
    ...
    for mdl in sys.models:
        ...
        for v in mdl.states:
            for i, address in enumerate(v.a):
                var_name = v.name + "_" + mdl.name + "_" + str(i)
